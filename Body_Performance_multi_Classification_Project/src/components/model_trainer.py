...
class ModelTrainer:
    ...
    def set_models_clf(self):
        try:
            if const.PRED_TYPE == 'multi_classification':
                models = {
                    ...
                    "CatBoost": OneVsRestClassifier(CatBoostClassifier(task_type="CPU", allow_writing_files=False, verbose=False)),
                ...
                models = {
                    ...
                    "CatBoost": CatBoostClassifier(task_type="CPU", allow_writing_files=False, verbose=False),
    ...
    def initiate_model_training_clf(self) -> MTArtifact:
        try:
            ...
            if const.PRED_TYPE == 'multi_classification':
                num_classes = const.NUM_MULTI_CLASS
                X_train, y_train, X_test, y_test = (
                    loaded_train_np_file[:, :-num_classes],
                    loaded_train_np_file[:, -num_classes:],
                    loaded_test_np_file[:, :-num_classes],
                    loaded_test_np_file[:, -num_classes:]
                )
            else:
                X_train, y_train, X_test, y_test = (
